...
class TabfileTestCase(TestCase):
    def setUp(self):
        self.fstab = resource_filename("serverscripts.tests", "example_fstab")
        self.fstab_with_duplicate_mount_point = resource_filename(
            "serverscripts.tests", "example_fstab_with_duplicate_mount_point"
        )
    ...
    def test_fstab(self):
        expected = {
            "/some/mount": {
                ...
                "options": {"credentials": "/etc/blabla.txt", "iocharset": "utf8"},
            }
        }
    ...
    def test_fstab_with_duplicate_mount_point(self):
        result = cifsfixer._cifs_lines(self.fstab_with_duplicate_mount_point)[0]
        # In case of duplicates, the second one wins. A warning is logged (but
        # we don't test that here).
        self.assertEqual(result["/some/mount"]["cifs_share"], "//someserver/something")
    ...
    def test_fstab_with_duplicate_mount(self):
        ...
        self.assertEqual(
            expected_keys,
            sorted(cifsfixer._cifs_lines(self.fstab_with_duplicate_mount)[0].keys()),
        )
...
class UtilsTestCase(TestCase):
    ...
    def test_check_unkown_mounts1(self):
        ...
        self.assertEqual(0, cifsfixer.check_unknown_mounts(fstab_mounts, mtab_mounts))
