...
def eggs_info(directory):
    ...
    for file_ in files_of_interest:
        ...
        output, error = get_output("%s --version" % python_executable, cwd=directory)
        try:
...
def parse_django_info(output):
    ...
    interesting = ["DEBUG", "DATABASES", "SETTINGS_MODULE"]
    lines = [
        line
        for line in output.split("\n")
        if "<" not in line and "datetime" not in line and line
    ]
    lines = [line for line in lines if line.split()[0] in interesting]
    output = "\n".join(lines)
    ...
    for database in settings.get("DATABASES", {}).values():
        ...
        if "spatialite" in engine or "sqlite" in engine:
            result["databases"].append(
                {"name": "local sqlite/spatialite file", "host": "localhost"}
            )
        elif "post" in engine:
            result["databases"].append(
                {
...
def supervisorctl_warnings(supervisorctl_command):
    ...
    if num_not_running:
        logger.warning("Some processes in %s aren't running:", supervisorctl_command)
...
def main():
    ...
    for name in os.listdir(SRV_DIR):
        ...
        if os.path.isfile(directory):
            logger.info("Ignoring %s, it is a file (*.tgz, for instance)", directory)
            continue
        if name == "lost+found":
        ...
        if os.path.exists(os.path.join(directory, "Pipfile")) and whereis("pipenv"):
            mode = "pipenv"
        ...
        if mode == "buildout":
            ...
            if os.path.exists(etc_directory):
                confs = [
                    fn
                    for fn in os.listdir(etc_directory)
                ...
                if len(confs) == 1:
                    ...
                    logger.exception(
                        "No supervisorctl configuration found in %s", etc_directory
                    )
                else:
                    logger.exception(
